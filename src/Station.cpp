...
bool Station::spoorBezet(int spoor) const {
    ...
    return trams.at(spoor) != nullptr;
}

void Station::bezetSpoor(int spoor, Tram *tram) {
    ...
    REQUIRE(tram->properlyInitialised(), "Station was niet geinitialiseerd bij de aanroep van bezetSpoor.");
    trams.at(spoor) = tram;
    ENSURE(spoorBezet(spoor), "Spoor was niet correct bezet na aanroep van bezetSpoor");
}