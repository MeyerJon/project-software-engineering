...
bool Metronet::checkConsistent(Exporter* exp) {
    ...
    for (Tram* tram : trams) {
        if (find(stations.begin(), stations.end(), tram->getBeginStation()) == stations.end()) {
            std::string out = "ERROR: Tram " + std::to_string(tram->getSpoor()->getLijnNr()) + " heeft een ongeldig beginstation.";
            consistent = false;
        }
        tramSporen.push_back(tram->getSpoor());
        if (find(stationSporen.begin(), stationSporen.end(), tram->getSpoor()) == stationSporen.end()) {
            std::string out = "ERROR: Spoor " + std::to_string(tram->getSpoor()->getLijnNr()) + " komt niet door een station.";
            consistent = false;
        }
    }
    ...
    for (Spoor* spoor : sporen) {
        if (find(tramSporen.begin(), tramSporen.end(), spoor) == tramSporen.end()) {
            std::string out = "ERROR: Spoor " + std::to_string(spoor->getLijnNr()) + " heeft geen tram.";
            consistent = false;
        }
    }
